...
use nom::branch::alt;
use nom::bytes::complete::{tag, take, take_till, take_till1, take_until, take_until1, take_while, take_while1};
...
impl<'a> Arg<'a> {
    fn parse(input: &'a str) -> IResult<&str, Self> {
        ...
        Ok((input, Self { arg_name, arg_binding }))
    }
}
...
impl<'a> Expr<'a> {
    fn unwrap_lit_str(&self) -> &LitStr<'a> {
        match self {
...
impl<'a> Expr<'a> {
    fn parse(input: &'a str) -> IResult<&str, Self> {
        ...
        let (input, expr) = alt((
            map_res(ws(Add::parse), |add| Ok::<Expr<'_>, &str>(Expr::Add(Box::new(add)))),
...
pub struct SideEffectStmt<'a> {
    side_effect_name: &'a str,
    side_effect_arguments: Vec<Expr<'a>>,
...
impl<'a> SideEffectStmt<'a> {
    fn parse(input: &'a str) -> IResult<&str, Self> {
        ...
        let (_input, side_effect_arguments) = separated_list0(
            ws(tag(",")),
            ws(Expr::parse)
        )(args)?;

        println!("side effect side_effect_arguments = {:?}", side_effect_arguments);
        println!("side effect input = {:?}", input);
        let (input, binding) = opt(preceded(ws(tag("as")), identifier))(input)?;
        Ok((input, Self { side_effect_name, side_effect_arguments, binding }))
    }
}

#[derive(Debug, Clone)]
pub struct SideEffects<'a> {
    side_effect_stmts: Vec<SideEffectStmt<'a>>,
}

impl<'a> SideEffects<'a> {
    fn parse(input: &'a str) -> IResult<&str, Self> {
        let (input, _) = ws(tag("("))(input)?;
        let (input, side_effect_stmts) = separated_list0(
            ws(tag(",")),
            SideEffectStmt::parse,
        )(input)?;
        let (input, _) = ws(tag(")"))(input)?;
...
impl<'a> DeclareMacro<'a> {
    fn parse(input: &'a str) -> IResult<&str, Self> {
        ...
        println!("args: {:?}", args);
        println!("input: {:?}", input);
        let (input, side_effects) = preceded(
            ws(tag("side_effects=")),
            SideEffects::parse,
        )(input)?;
        ...
        Ok((input, Self { args: args.unwrap_or_default(), side_effects, returns }))
    }
}
...
mod tests {
    ...
    fn test_side_effect_stmt() {
        ...
        assert_eq!(side_effect_stmt.side_effect_arguments[0].unwrap_var().name, "bar");
        assert_eq!(side_effect_stmt.side_effect_arguments[1].unwrap_var().name, "baz");
    ...
    fn test_side_effects_parse() {
        ...
        assert_eq!(side_effect_stmt.side_effect_arguments[0].unwrap_add().lhs.unwrap_var().name, "T");
        assert_eq!(side_effect_stmt.side_effect_arguments[0].unwrap_add().rhs.unwrap_lit_str().value, "/");
        assert_eq!(side_effect_stmt.binding, Some("U"));

        let side_effect_stmt = &side_effects.side_effect_stmts[1];
        assert_eq!(side_effect_stmt.side_effect_name, "eval");
        assert_eq!(side_effect_stmt.side_effect_arguments.len(), 1);
        assert_eq!(side_effect_stmt.side_effect_arguments[0].unwrap_var().name, "T");
    ...
    fn test_expr_nested_parens() {
        ...
        assert_eq!(rest, "");
        assert_eq!(expr.unwrap_add().lhs.unwrap_add().lhs.unwrap_var().name, "T");
        assert_eq!(expr.unwrap_add().lhs.unwrap_add().rhs.unwrap_lit_str().value, "/");
    ...
    fn test_declare_macro_parse() {
        ...
        assert_eq!(declare_macro.side_effects.side_effect_stmts[0].side_effect_name, "eval");
        assert_eq!(declare_macro.side_effects.side_effect_stmts[0].side_effect_arguments.len(), 1);
        assert_eq!(declare_macro.side_effects.side_effect_stmts[0].side_effect_arguments[0].unwrap_add().lhs.unwrap_var().name, "F");
        assert_eq!(declare_macro.side_effects.side_effect_stmts[0].side_effect_arguments[0].unwrap_add().rhs.unwrap_lit_str().value, "/");
        assert_eq!(declare_macro.side_effects.side_effect_stmts[0].binding, Some("FS"));
        assert_eq!(declare_macro.side_effects.side_effect_stmts[1].side_effect_name, "eval");
        assert_eq!(declare_macro.side_effects.side_effect_stmts[1].side_effect_arguments.len(), 1);
        assert_eq!(declare_macro.side_effects.side_effect_stmts[1].side_effect_arguments[0].unwrap_add().lhs.unwrap_var().name, "FS");
        assert_eq!(declare_macro.side_effects.side_effect_stmts[1].side_effect_arguments[0].unwrap_add().rhs.unwrap_var().name, "B");
        assert_eq!(declare_macro.side_effects.side_effect_stmts[1].binding, Some("result"));
        assert_eq!(declare_macro.side_effects.side_effect_stmts[2].side_effect_name, "read_file");
        assert_eq!(declare_macro.side_effects.side_effect_stmts[2].side_effect_arguments.len(), 1);
        assert_eq!(declare_macro.side_effects.side_effect_stmts[2].side_effect_arguments[0].unwrap_var().name, "result");
        assert_eq!(declare_macro.side_effects.side_effect_stmts[2].binding, None);
    }
